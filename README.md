...
- 44 个开发、文件处理、计算、效率和移动开发速查工具
- 中英文界面、深色/浅色主题
- 首页搜索，以及拥有独立可刷新路由的“收藏 / 分类 / 推荐”三 Tab
- 桌面端三级侧栏；移动端顶部栏 + 抽屉导航
...
| 首页分类 | 工具 |
|----------|------|
| 开发调试 | 设备信息、JSON 工具、代码格式化、正则测试、URL 解析、HTTP 速查、Curl、Cron 计算器 |
| 编码安全 | 编码转换、Base64、UUID / ULID、AES/RSA 加解密、JWT、二维码 |
| 文本文件 | 文本处理、代码对比、Markdown 编辑、图片处理、文件转换、文件信息 |
| 数据计算 | 数据可视化、函数绘图、时间戳、单位换算、颜色转换、汇率计算、税收计算、房贷计算 |
| 开发速查 | 终端命令、Git、AI 指令、Android、Flutter、iOS |
| 小游戏 | 专注力训练、万象小球、捕食与繁衍、多方循环、红绿大战、大鱼吃小鱼 |
| 日常效率 | 内容分享、翻译、地区搜索 |
...
| 标识 | 含义 | 示例 |
|------|------|------|
| 本地 | 数据只在当前浏览器处理 | 设备环境报告、JSON、图片、文件转换、JWT、加解密 |
| 混合 | 页面本地运行，但部分信息请求服务端 | 地区搜索、汇率计算 |
...
- 首页：`/{zh|en}/`
- 首页 Tab：`/{zh|en}/{favorites|categories|recommended}`
- 工具页：`/{zh|en}/tool/{id}`
