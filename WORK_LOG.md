...
| 2026-07-14 | 透明度转换 | 颜色转换新增独立“透明度转换”小 Tab；支持输入不透明度或透明度并实时联动，输出百分比、CSS Alpha、8-bit 十进制、Alpha Hex、HEX ARGB（AARRGGBB）与 CSS RGBA，同时同步当前颜色的 Alpha 和透明效果预览；8 位 HEX 输入与显示统一采用 ARGB 顺序，不输出 RRGGBBAA。 |
