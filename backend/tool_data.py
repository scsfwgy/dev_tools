...
TOOLS = {
    ...
    "ball-game": {
        "zh": {
            ...
            "description": "默认从一个小球开始，可配置数量、颜色、恒定速度、数量上限及相撞消失；每次撞墙都会一分为二、对称发射，并按代际统一变色。",
            "keywords": "小球游戏,撞墙分裂,指数裂变,随机动画,代际变色,Canvas游戏,生命模拟,在线沙盒",
            "intro": "默认从画布正中心随机发射一颗种子球，看它每次撞墙后分裂为两个，以对称角度重新发射；每一代共享一种纯色，所有小球的大小和速度始终不变。",
            "features": ["初始球从正中心随机发射", "撞墙指数裂变", "相撞同时消失且可关闭", "空间网格碰撞检测", "同代统一纯色并逐代变色", "固定大小与 0.1×–10× 恒定速度", "毫秒级总时间与达上限用时", "时间—数量实时变化曲线", "可自由输入上限并即时校验", "小球画布全屏", "无拖尾、无网格的本地 Canvas 动画"],
            "faq": [
                ...
                ("小球的大小或速度会变化吗？", "不会。所有小球使用相同的固定半径，并在整次运行中保持开始前设定的速度。"),
        ...
        "en": {
            ...
            "description": "Start with one ball or configure count, color, constant speed, population limit and disappear-on-contact behavior. Every wall hit splits it into two symmetric paths.",
            ...
            "faq": [
                ...
                ("Do ball size or speed change?", "No. Every ball uses the same fixed radius and keeps the speed selected before the run."),
