...
Tools24 / DevTools 是简洁、快速、注重隐私的在线开发者工具箱，部署于 `https://dev.tools24.uk`。当前提供 36 个公开工具，覆盖数据与编码、文本与文件、安全与网络、计算与效率，以及 Android / Flutter / iOS 开发速查。
...
首页提供工具搜索，以及“收藏 / 分类 / 推荐”三 Tab。存在收藏时默认打开收藏 Tab，否则默认打开分类 Tab；分类包含全部、开发调试、编码安全、文本文件、计算效率、平台速查和日常工具。推荐 Tab 按投资理财、资源聚合、大模型、科学上网和加密交易分组展示外部资源。收藏仅保存在当前浏览器 `localStorage`。
