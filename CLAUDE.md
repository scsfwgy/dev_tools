...
Tools24 / DevTools 是简洁、快速、注重隐私的在线开发者工具箱，部署于 `https://tools24.uk`。当前提供 30 个公开工具，覆盖数据与编码、文本与文件、安全与网络、计算，以及 Android / Flutter / iOS 开发速查。

首页提供工具搜索，以及“收藏 / 分类”双 Tab。存在收藏时默认打开收藏 Tab，否则默认打开分类 Tab；分类包含全部、文件处理、换算计算、数据调试、编码安全、移动开发、开发速查和系统服务。收藏仅保存在当前浏览器 `localStorage`。
...
- `frontend/js/app.js` — i18n、History 路由、首页双 Tab、收藏、工具分类、菜单状态与工具脚本懒加载
